  ...
  return (
    <Typography style={styles.display}>
      <Paragraph style={styles.statusText}>
        Address:{" "}
        {!isTablet ? (
          <span style={styles.statusValue}>{process.env.REACT_APP_CONTRACT_ADDRESS}</span>
        ) : (
          <span style={styles.statusValue}>
            {process.env.REACT_APP_CONTRACT_ADDRESS && getEllipsisTxt(process.env.REACT_APP_CONTRACT_ADDRESS, 4)}
          </span>
        )}
      </Paragraph>
